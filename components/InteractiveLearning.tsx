...
const SimPhishing = () => {
    ...
    return (
        <SimulationStage title="Email Client - Inbox (1 Unread)" color="border-red-500/30">
            <div className="flex flex-col gap-4 w-full max-w-md h-full justify-center">
                <div className="bg-white text-black p-5 rounded-lg border-l-4 border-red-500 shadow-2xl relative font-sans">
                    <div className="border-b border-gray-200 pb-2 mb-2 flex justify-between items-start">
                        <div>
                            ...
                            <div className="text-xs text-gray-500 group relative cursor-help">
                                From: support@secure-bank-verify.co.nz
                                <div className="absolute top-full left-0 bg-black text-white text-[10px] p-1 rounded opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity">External Domain Warning</div>
                            </div>
                        </div>
                        <div className="text-[10px] text-gray-400">10:42 AM</div>
                    </div>
                    <p className="text-sm mb-4 leading-relaxed text-gray-700">
                        Dear User,<br/><br/>
                        We detected unusual activity. Click here immediately to verify your identity or lose access forever.
                    </p>
                    <div className="bg-blue-50 p-2 rounded text-center border border-blue-100 cursor-pointer hover:bg-blue-100 transition-colors">
                        <span className="text-blue-600 text-xs font-bold underline">http://secure-bank-verify.co.nz/login</span>
                    </div>
                </div>
                
                <AnimatePresence>
                    {result ? (
                        <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} className={`p-3 rounded-lg text-center text-sm font-bold border ${result.includes("Correct") ? "bg-green-500/10 border-green-500 text-green-400" : "bg-red-500/10 border-red-500 text-red-400"}`}>
                            {result}
                            <button onClick={() => setResult("")} className="ml-2 underline text-xs opacity-70">Reset</button>
                        </motion.div>
                    ) : (
                        <div className="flex gap-4 justify-center">
                            <button onClick={() => setResult("Correct! This is a phishing attempt.")} className="px-4 py-2 bg-red-500 hover:bg-red-600 rounded-lg text-white font-bold text-xs flex items-center gap-2">
                                <AlertTriangle size={14}/> Report Phishing
                            </button>
                            <button onClick={() => setResult("Incorrect. Look at the URL and urgency.")} className="px-4 py-2 bg-slate-700 hover:bg-slate-600 rounded-lg text-white font-bold text-xs">
                                Trust Email
                            </button>
                        </div>
                    )}
                ...
                <div className="flex-1 p-4 space-y-3 overflow-y-auto custom-scrollbar">
                    {chat.map((c, i) => (
                        <div key={i} className={`flex ${c.sender === 'You' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[85%] p-3 rounded-xl text-xs md:text-sm ${
                                c.sender === 'You' ? 'bg-blue-600 text-white rounded-tr-none' : 
        ...
        <SimulationStage title="Eisenhower Matrix Board" color="border-indigo-500/30">
            <div className="w-full max-w-lg h-full flex flex-col p-2">
                <div className="grid grid-cols-2 gap-2 flex-1 mb-2">
                    {[1,2,3,4].map((q, i) => (
                        <div key={q} className={`border rounded-lg p-2 relative flex flex-col ${
                            q === 1 ? 'border-red-500/30 bg-red-500/5' : 
...
export const InteractiveLearning: React.FC<InteractiveLearningProps> = ({ simulationId, onClose, onNavigate }) => {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-50 pt-24 bg-slate-950/95 backdrop-blur-3xl flex flex-col"
    >
                ...
                {moduleData.chapters.map((_, i) => (
                    <div 
                        ...
                        className={`flex-1 rounded-full transition-all duration-500 ${
                            i < currentChapterIndex ? moduleData.color : 
            ...
            {isCompleted ? (
                ...
                >
                    ...
                    <div className="flex flex-col justify-center order-2 lg:order-1 h-full overflow-y-auto custom-scrollbar p-4 lg:pr-8">
                            ...
                            <div className="space-y-3 w-full">
                                ...
                                {currentChapter.quizData.options.map((opt, i) => (
                                    <button
                                        ...
                                        className={`w-full p-4 rounded-xl border text-left transition-all font-semibold flex items-center justify-between group ${
                                            quizAnswer === i
                        ...
                        <div className="hidden lg:flex items-center gap-4 mt-auto pt-8 border-t border-white/5">
                             <button
                                onClick={handlePrev}
                            ...
                            <button
                                ...
                                className={`flex-1 py-3 rounded-full font-bold text-white shadow-lg transition-all flex items-center justify-center gap-2 ${
                                    currentChapter.type === 'quiz' && !isQuizCorrect 
                    ...
                    <div className="order-1 lg:order-2 flex flex-col h-[400px] lg:h-auto relative">
                        ...
                        <div className="lg:hidden flex items-center gap-4 mt-4 mb-6">
                             <button
                                onClick={handlePrev}
                                disabled={currentChapterIndex === 0}
                                className="px-4 py-3 rounded-xl bg-slate-800 text-slate-400 disabled:opacity-30"
                            >
                                <ChevronLeft size={20}/>
                            </button>
                            <button
                            ...
                            >
                                {currentChapterIndex === moduleData.chapters.length - 1 ? 'Finish' : 'Next'} <ChevronRight size={16} />
                            </button>
                        </div>
                    </div>
                </motion.div>
