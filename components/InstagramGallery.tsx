...
const socialPlatforms = [
    ...
    { name: 'YouTube', icon: Icons.YouTube, color: 'text-red-500', url: 'https://youtube.com/@geniusphereofficial1231?si=GKq07XBAkJ_I-JQh' },
    { name: 'LinkedIn', icon: Icons.LinkedIn, color: 'text-blue-500', url: 'https://www.linkedin.com/in/geniusphere-the-students-startup-9067b12a9/' },
];
      ...
      `}</style>
            ...
            <div className="container mx-auto px-6">
                ...
                <motion.div
                    ...
                    className="mb-20"
                >
                    {/* Updated visual style to match Glassmorphism theme */}
                    <div className="relative rounded-3xl overflow-hidden bg-slate-900/60 backdrop-blur-xl border border-white/10 p-8 md:p-12 shadow-2xl">
                        {/* Background Blobs for specific "Geniusphere" look */}
                        ...
                        <div className="relative z-10">
                            <div className="text-center">
                                <div className="inline-flex items-center gap-2 px-4 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-xs font-bold text-cyan-300 uppercase tracking-wider mb-4">
                                    <Megaphone size={14} /> Recruitment Open
                                </div>
                                <h3 className="text-3xl md:text-5xl font-bold text-white mb-6">Genius Representatives Program</h3>
                                <p className="text-slate-300 text-lg mb-8 leading-relaxed max-w-2xl mx-auto">
                                    Become a leader at your college. Promote Geniusphere, conduct workshops, and earn exclusive rewards while building your resume.
                                </p>

                                <div className="flex flex-wrap gap-4 justify-center mb-10">
                                    <div className="flex items-center gap-2 text-sm text-slate-200 bg-black/40 px-5 py-3 rounded-xl border border-white/5 hover:border-white/20 transition-colors">
                                        <Gift size={18} className="text-pink-400" /> Earn Rewards
                                    </div>
                                    <div className="flex items-center gap-2 text-sm text-slate-200 bg-black/40 px-5 py-3 rounded-xl border border-white/5 hover:border-white/20 transition-colors">
                                        <Users size={18} className="text-blue-400" /> Host Events
                                    </div>
                                    <div className="flex items-center gap-2 text-sm text-slate-200 bg-black/40 px-5 py-3 rounded-xl border border-white/5 hover:border-white/20 transition-colors">
                                        <Trophy size={18} className="text-yellow-400" /> Leadership Certs
                                    </div>
                                </div>

                                <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                                    <button onClick={handleApply} className="px-8 py-3.5 bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white font-bold rounded-xl shadow-lg shadow-cyan-500/20 flex items-center gap-2 transition-all hover:scale-105">
                                        Apply to Program <ArrowRight size={18} />
                                    </button>
                                    <button
                                        onClick={() => setShowLeaderboard(true)}
                                        className="px-8 py-3.5 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-bold rounded-xl flex items-center gap-2 transition-all"
                                    >
                                        View Leaderboard <Crown size={18} className="text-yellow-400" />
                                    </button>
                                </div>
                            </div>
                        </div>
                ...
                <div className="relative group/carousel">
                    <div className="flex justify-between items-end mb-8">
                        <div>
                            ...
                            <p className="text-slate-400 text-sm">Join workshops, competitions, and more.</p>
                        </div>
                        <div className="flex gap-2">
                            <button
                                onClick={() => setShowAllEvents(!showAllEvents)}
                                className={`px-4 py-2 rounded-full border border-white/10 text-xs font-bold transition-all ${showAllEvents ? 'bg-blue-600 text-white' : 'bg-white/5 text-slate-300 hover:bg-white/10'}`}
                            >
                                {showAllEvents ? 'Close View' : 'View All'}
                            </button>
                            {!showAllEvents && (
                                <>
                                    <button onClick={() => scroll('left')} className="p-2 rounded-full bg-white/5 hover:bg-white/10 text-white border border-white/10 transition-colors">
                                        <ChevronLeft size={20} />
                                    </button>
                                    <button onClick={() => scroll('right')} className="p-2 rounded-full bg-white/5 hover:bg-white/10 text-white border border-white/10 transition-colors">
            ...
            {/* Leaderboard Modal (Next Page Interface) */}
            <AnimatePresence>
                {showLeaderboard && (
                    <motion.div
                        initial={{ opacity: 0 }}
                        ...
                        >
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 bg-slate-950/50">
                                <div className="space-y-4">
                                    {sortedAmbassadors.length > 0 ? sortedAmbassadors.map((amb, idx) => (
                                        <motion.div
                                            key={amb.id}
            ...
            <div className="container mx-auto px-6">
                ...
                <div className="flex flex-row flex-wrap items-center justify-center gap-4 md:gap-6">
                    {socialPlatforms.map((platform, i) => (
                        ...
                        >
                            <div className="mb-2 transition-colors duration-300 group-hover:${platform.color} text-slate-300">
                                <platform.icon />
                            </div>
